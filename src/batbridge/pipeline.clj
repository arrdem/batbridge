...
(t/defn> fetch 
  ...
  (let [pc (get-register processor 31)
        icode (or (get-memory processor pc) i/vec-no-op)]
    (info (format "[fetch    ] pulling from address %d : %s" pc icode))
    (-> processor
...
(t/defn> decode
  ...
  (let [icode (get processor :fetch i/vec-no-op)
    ...
    (if (and (i/fetched? icode)
      ...
      (do (info (format "[decode   ] data hazard detected! setting pc to %d..."
                           (:fetch-pc (meta icode))))
          (-> processor
              (assoc :decode i/map-no-op)
              (assoc-in [:registers 31] (:fetch-pc (meta icode)))))
      (do (info "[decode   ] no data hazard.. decoding.. done.")
...
(t/ann opcode->fn (t/Map t/AnyInteger
...
(t/defn> execute
  ...
  (let [icode (get processor :decode i/map-no-op)
    ...
    (when-not (i/fetched? icode)
      (info "[execute  ] processing no-op..."))
    (assoc processor :execute
           (with-meta
             (case opcode
               (:add :sub :ld)
                 (do (info "[execute  ] processing registered op...")
                     [:registers dst ((get opcode->fn opcode (constantly 0))
                                      srca srcb processor)])

               (:st)
                 (do (info "[execute  ] processing store op...")
                     [:memory (+ srca (* 4 srcb)) dst])

               (do (info "[execute  ] sending halt...")
                   [:halt]))
             (meta icode)))))
...
(t/defn> writeback 
  :- i/Processor
  [processor :- i/Processor]
  (let [directive (get processor :execute (with-meta 
                                            [:registers 0 0]
                                            {:fetched? false}))]
    (when-not (i/fetched? directive)
      (info "[writeback] processing no-op..."))

    (cond ;; special case to stop the show
          (= :halt (first directive))
            (do (info "[writeback] halting...")
          ...
          true
            (do (info "[writeback] processing register store...")
...
(defn run [state & {:keys [limit]
                    :as kwargs
                    :or {limit 50}}]
  (t/loop> [state :- i/Processor state
            i :- t/AnyInteger limit]
